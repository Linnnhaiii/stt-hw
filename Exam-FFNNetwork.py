...
class FNN:
    def __init__(self, n_inputs, hidden_sizes=[2,3,2], n_outputs=1):
        self.nx = n_inputs
        self.ny = n_outputs
...
if __name__ == "__main__":
    ...
    # y_train = np.expand_dims(Y_train,1).toarray()
    ...
    ffnet = FNN(2, [5])
    ffnet.fit(X_train, Y_train, epochs=1000, learning_rate=0.001, display_loss=True)
    ...
    print("Training accuracy-Exam", round(accuracy_train, 2))
    print("Testing accuracy-Exam", round(accuracy_test, 2))

    my_cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", "blue", "green"])
    plt.figure()
    plt.scatter(X_train[:,0], X_train[:,1], c=Y_pred_binarised_train, cmap=my_cmap, s=15*(np.abs(Y_pred_binarised_train-Y_train)+.2))
    plt.savefig('Exam-boundary.png')