...
class FNN:
    ...
    # initialise w, b, standard normal...
        if initialise:
            for i in range(self.nh+1):
                self.W[i+1] = np.random.randn(self.sizes[i], self.sizes[i+1])
                self.B[i+1] = np.zeros((1, self.sizes[i+1]))
...
if __name__ == "__main__":
    X_train = np.loadtxt('./Iris/train/x.txt', dtype='str').astype(np.float64)
    X_test = np.loadtxt('./Iris/test/x.txt', dtype='str').astype(np.float64)
    Y_train = np.loadtxt('./Iris/train/y.txt', dtype='str').astype(np.int64)
    Y_test = np.loadtxt('./Iris/test/y.txt', dtype='str').astype(np.int64)
